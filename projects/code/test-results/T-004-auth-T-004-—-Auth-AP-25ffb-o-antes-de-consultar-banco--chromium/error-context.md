...
- Name: T-004-auth.spec.ts >> T-004 — Auth API >> POST /auth/reset-password >> token presente + senha curta → 400 de validação (antes de consultar banco)
- Location: tests/e2e/T-004-auth.spec.ts:296:9
...
Call log:
  - → POST http://localhost:3001/api/v1/auth/reset-password?token=tokeninvalido
